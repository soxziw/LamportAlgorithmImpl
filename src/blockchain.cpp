...
void Blockchain::printBlockchain() {
    for (int i = 0; i < blockchain_.size(); i++) {
        std::printf("[Server %d] Block %d. Operation: %s; Previous Hash: %s; Hash: %s\n", LamportServer::getInstance()->client_id_, i, blockchain_[i].operation.opr.c_str(), blockchain_[i].prev_hash.c_str(), blockchain_[i].hash.c_str());
    }
}

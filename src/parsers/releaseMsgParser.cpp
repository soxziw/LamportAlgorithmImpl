...
std::string ReleaseMsgParser::stringify(std::unique_ptr<Msg>&& msg) {
    // std::printf("[Server %d] Stringify ReleaseMsg.\n", LamportServer::getInstance()->client_id_);
    ReleaseMsg* msg_raw = dynamic_cast<ReleaseMsg*>(msg.get());
    if (!msg_raw) { // Could not cast
        std::printf("\033[31m[Error][ReleaseMsgParser::stringify][Server %d] message does not fit in ReleaseMsg.\033[0m\n", LamportServer::getInstance()->client_id_);
    }
...
std::unique_ptr<Msg> ReleaseMsgParser::parse(const std::string& str) {
    // std::printf("[Server %d] Parse ReleaseMsg.\n", LamportServer::getInstance()->client_id_);

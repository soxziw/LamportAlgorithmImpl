...
int TransferTransReqProcessor::process(std::unique_ptr<Msg>&& msg){
    ...
    lamport_client_ptr->replys_ = std::vector<bool>(lamport_client_ptr->balance_tb_.size(), true);
    ...
    lamport_client_ptr->finishes_ = std::vector<bool>(lamport_client_ptr->balance_tb_.size(), true);
